...
class CookieController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    {
        if (array_key_exists("statisticsDays", $this->settings)) {
            $statisticDay = (int)$this->settings['statisticsDays'];
        } else {
            $statisticDay = 0;
        }
        ...
        if (strpos((string) $cookie, 'setwcm')===0) {
            if ($statisticDay>0) {
                ...
                if ($cookieneu=='min') {
                    $newStat->setCookieconfig('min');
                } elseif ($cookieneu=='max') {
                    $newStat->setCookieconfig('max');
                } else {
                    $newStat->setCookieconfig('custom');
                }
        ...
        if ($cookiearray['0']=="max") {
            ...
            foreach ($cookies1 as $check) {
                if (in_array($check->getUid(), $cookies)) {
                    $cookie1check = 1;
                }
            }
            $cookie2check = 0;
