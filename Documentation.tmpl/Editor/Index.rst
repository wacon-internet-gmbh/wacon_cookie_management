.. include:: ../Includes.txt

.. _for-editors:

===========
For Editors
===========

Manage cookies
==============
Manage your cookies by selecting the corresponding folder in the List module:

Create or edit a cookie record and supply the required information:

- Short description 
- Cookie name
- Category (Essential / Statistics / Marketing)
- Vendor
- Purpose
- Name
- Duration
- Link to Privacy Policy
- Server, that set the cookie



Use Plugin
==========
Prevent external content to load without consent by using the plugin "Include external ressource":

.. figure:: ../Images/wcm_screenshot9.jpg
   :class: with-shadow
   :alt: Plugin "Include external ressource"
   :width: 300px

	Include external ressource
	
Fill out the fields:

- Choose the corresponding Cookie record
- Copy the script or html code in the field "Script"
- Deliver text to be shown if the ressource is not accepted yet by the visitor
- And/or deliver an image to be shown if the ressource is not accepted yet by the visitor ("Frosted glas effect")
- Tell the Plugin where your Cookie records are stored

.. figure:: ../Images/wcm_screenshot10.jpg
   :class: with-shadow
   :alt: Information of an external ressource
   :width: 500px

	Define external ressource
	
Show statistics
==========
Since TYPO3 v10 you can analyze the usage of the cookie settings:

.. figure:: ../Images/wcm_screenshot4.jpg
   :class: with-shadow
   :alt: Dashboard integration
   :width: 500px

Click on "details" to select the information by month:
	
.. figure:: ../Images/wcm_screenshot11.jpg
   :class: with-shadow
   :alt: Cookie counter
   :width: 500px
